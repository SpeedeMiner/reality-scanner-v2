...
func classifySNIByDNS(ctx context.Context, host, targetIP string) dnsSNIClass {
	ctx, cancel := context.WithTimeout(ctx, 1800*time.Millisecond)
	defer cancel()

	ips, err := net.DefaultResolver.LookupIP(ctx, "ip", host)
	if err != nil || len(ips) == 0 {
		// A DNS timeout/error is not proof that the SNI is fake. Do not
		// promote an unverified name into the fake-SNI result set.
...
func fakeSNINames(ctx context.Context, ip string, names []string) []string {
	...
	for _, name := range uniqueSorted(names) {
		...
		if classifySNIByDNS(ctx, name, ip) != dnsSNIDoesNotMatchTarget {
			continue
		}
		out = append(out, name)
	}
